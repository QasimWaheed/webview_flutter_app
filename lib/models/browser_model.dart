...
class BrowserModel extends ChangeNotifier {
  ...
  void addTab(WebViewTab webViewTab) {
    ...
    print(webViewTab.webViewModel.title);
  ...
  bool containsFavorite(FavoriteModel favorite) {
    return _favorites.contains(favorite) || _favorites.any((element) => element.url == favorite.url);
  }
  ...
  void removeFavorite(FavoriteModel favorite) {
    if (!_favorites.remove(favorite)) {
      var favToRemove = _favorites.firstWhere((element) => element.url == favorite.url);
      if (favToRemove != null) {
